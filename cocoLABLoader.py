...
class CocoDataSet(Dataset):
    def __init__(self, annt_file, data_dir=DATAPATH, cats=None, size=5000, transform=None):
        ...
        self.filenames = [x for x in imgIds if x not in GRAYS]
    ...
    def __getitem__(self, index):
        ...
        img = read_image(self.data_dir + '{:012}'.format(self.filenames[index]) + '.jpg').to(torch.float32) / 255
        if self.transform:
            img = self.transform(img)
        img = img.permute(1, 2, 0)
        img = torchvision.transforms.Lambda(lambda img: cv2.cvtColor(img.numpy(), cv2.COLOR_RGB2LAB))(img)
        img = torch.Tensor(img).permute(2, 0, 1)
        return img



def load_coco_lab_dataset(batch_size=64, cats=None, size=123176, dim=64):
